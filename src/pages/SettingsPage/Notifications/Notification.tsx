...
const NotificationsSounds = () => {
  ...
  useEffect(() => {
    const fetchUserSound = async () => {
      ...
      const soundRef = doc(db, 'users', userId);
      const soundDoc = await getDoc(soundRef);
      
      if (soundDoc.exists()) {
        const userSound = soundDoc.data().soundURL;
        setSelectedSound(userSound);
      } else {
        setSelectedSound(soundOptions[0].url); // Default sound if no selection is stored
      }
    };
  ...
  const handleSoundSelection = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (soundDoc.exists()) {
        await updateDoc(soundRef, {
          soundName: soundOptions.find((sound) => sound.url === selectedSoundUrl)?.label,
...
export default NotificationsSounds;
